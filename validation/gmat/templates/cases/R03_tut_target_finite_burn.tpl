%----------------------------------------
% R03: Tutorial Target Finite Burn Regression Test
% Adapted from Tut_Target_Finite_Burn.script
%
% Simplified tutorial version of targeted finite burn
%----------------------------------------
...
%----------------------------------------
Create FiniteBurn TutBurn;
GMAT TutBurn.Thrusters = {Thruster1};
...
Target DC {{"{"}}SolveMode = Solve, ExitMode = SaveAndContinue{{"}"}};
    ...
    BeginFiniteBurn TutBurn({{ spacecraft_name }});
    Propagate {{ propagator_name }}({{ spacecraft_name }}) {{"{"}}{{ spacecraft_name }}.ElapsedSecs = BurnDuration{{"}"}};
    EndFiniteBurn TutBurn({{ spacecraft_name }});
    Propagate {{ propagator_name }}({{ spacecraft_name }}) {{"{"}}{{ spacecraft_name }}.Apoapsis{{"}"}};
    Achieve DC({{ spacecraft_name }}.Altitude = {{ target_altitude_km | default(550) }}, {{"{"}}Tolerance = 0.1{{"}"}});
EndTarget;
