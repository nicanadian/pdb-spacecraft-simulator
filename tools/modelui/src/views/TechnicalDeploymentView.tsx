...
import {
  visibleNodes, visibleEdges, setLayoutNodes, layoutNodes,
} from '../stores/modelStore';
import { computeLayout } from '../services/layoutEngine';
import type { LayoutNode } from '../types/model';

/**
 * Technical deployment view reuses the force-directed GraphCanvas
 * from the original ArchitectureView, showing L2+L3+L4 Technical domain.
 */
export default function TechnicalDeploymentView() {
  ...
  createEffect(() => {
    ...
    const { stop } = computeLayout(ns, es, 1200, 800, (nodes: LayoutNode[]) => setLayoutNodes(nodes));
    stopLayout = stop;
  });
